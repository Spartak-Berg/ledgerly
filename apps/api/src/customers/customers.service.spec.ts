...
describe('CustomersService', () => {
  ...
  it('creates a customer', async () => {
    ...
    await expect(
      service.create({ companyName: customer.companyName }),
  ...
  it('throws when a customer does not exist', async () => {
    ...
    await expect(service.findOne(customer.id)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('updates an existing customer', async () => {
    ...
    await expect(
      service.update(customer.id, { status: CustomerStatus.ARCHIVED }),
  ...
  it('deletes an existing customer', async () => {
    ...
    await expect(service.remove(customer.id)).resolves.toBeUndefined();
  });
});
